...
class OneShipStation_XmlServiceTest extends BaseTest
{
    ...
    public function testModelMappingWithClosureReturnsCorrectFieldValue() {
        $object = new \StdClass();
        $expected_value = md5(time());

        $cdata_method = $this->getMethod(craft()->oneShipStation_xml, 'valueFromMappingAndModel');
        $callback = function($object) use ($expected_value) { return $expected_value; };
        $returned_value = $cdata_method->invoke(craft()->oneShipStation_xml, ['callback' => $callback], $object);

        $this->assertEquals($returned_value, '<![CDATA[' . $expected_value . ']]>');
    }

    public function testModelMappingWithClosureAndNoCDATAReturnsCorrectFieldValue() {
        $object = new \StdClass();
        $expected_value = md5(time());

        $cdata_method = $this->getMethod(craft()->oneShipStation_xml, 'valueFromMappingAndModel');
        $callback = function($object) use ($expected_value) { return $expected_value; };
        $returned_value = $cdata_method->invoke(craft()->oneShipStation_xml, ['cdata' => false, 'callback' => $callback], $object);

        $this->assertEquals($returned_value, $expected_value);
    }
